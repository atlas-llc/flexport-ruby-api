  ...
  class ApiProductResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::ApiProductResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::ApiProductResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @merchant_sku.to_s.length > 255
        invalid_properties.push('invalid value for "merchant_sku", the character length must be smaller than or equal to 255.')
      end

      if @merchant_sku.to_s.length < 1
        invalid_properties.push('invalid value for "merchant_sku", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @merchant_sku.to_s.length > 255
      return false if @merchant_sku.to_s.length < 1
    ...
    # Custom attribute writer method with validation
    # @param [Object] merchant_sku Value to be assigned
    def merchant_sku=(merchant_sku)
      if merchant_sku.nil?
        fail ArgumentError, 'merchant_sku cannot be nil'
      end

      if merchant_sku.to_s.length > 255
        fail ArgumentError, 'invalid value for "merchant_sku", the character length must be smaller than or equal to 255.'
      end

      if merchant_sku.to_s.length < 1
        fail ArgumentError, 'invalid value for "merchant_sku", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [name, logistics_sku, merchant_sku, barcodes, dimensions, international, dims_locked, created_at, updated_at].hash
    end
