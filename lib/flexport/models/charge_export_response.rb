...
module Flexport
  class ChargeExportResponse
    attr_accessor :signed_url

    attr_accessor :expiration_time

    # A year month date string in ISO 8601 format (YYYY-MM).
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::ChargeExportResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::ChargeExportResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @year_month.to_s.length > 7
        invalid_properties.push('invalid value for "year_month", the character length must be smaller than or equal to 7.')
      end

      if @year_month.to_s.length < 7
        invalid_properties.push('invalid value for "year_month", the character length must be great than or equal to 7.')
      end

      pattern = Regexp.new(/^[1-9][0-9]{3}-[0-1][0-9]$/)
      if @year_month !~ pattern
        invalid_properties.push("invalid value for \"year_month\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if @year_month.to_s.length > 7
      return false if @year_month.to_s.length < 7
      return false if @year_month !~ Regexp.new(/^[1-9][0-9]{3}-[0-1][0-9]$/)
      true
    end
    ...
    def year_month=(year_month)
      ...
      if year_month.to_s.length > 7
        fail ArgumentError, 'invalid value for "year_month", the character length must be smaller than or equal to 7.'
      end

      if year_month.to_s.length < 7
        fail ArgumentError, 'invalid value for "year_month", the character length must be great than or equal to 7.'
      end

      pattern = Regexp.new(/^[1-9][0-9]{3}-[0-1][0-9]$/)
      if year_month !~ pattern
        fail ArgumentError, "invalid value for \"year_month\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [signed_url, expiration_time, year_month].hash
    end
