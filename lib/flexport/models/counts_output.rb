  ...
  class CountsOutput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::CountsOutput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::CountsOutput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'sellable')
        self.sellable = attributes[:'sellable']
      else
        self.sellable = nil
      end

      if attributes.key?(:'damaged')
        self.damaged = attributes[:'damaged']
      else
        self.damaged = nil
      end
    ...
    def list_invalid_properties
      ...
      if @sellable.nil?
        invalid_properties.push('invalid value for "sellable", sellable cannot be nil.')
      end

      if @damaged.nil?
        invalid_properties.push('invalid value for "damaged", damaged cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @sellable.nil?
      return false if @damaged.nil?
    ...
    def hash
      [sellable, damaged, expected].hash
    end
