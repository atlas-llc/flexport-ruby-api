...
module Flexport
  # Input to create a new channel
  class CreateChannelInput
    # Name of the channel E.g. SHOPIFY
    attr_accessor :channel_type

    # Access token that can be used to connect to a Channel's API
    attr_accessor :access_token

    # Name of the authorized store that will be used to access a Channel's API E.g. shop1.myshopify.com
    attr_accessor :store_name

    # The Location Id where orders will be fetched from and inventory updates will occur at E.g. 1203908129
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::CreateChannelInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::CreateChannelInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [channel_type, access_token, store_name, location_id].hash
    end
