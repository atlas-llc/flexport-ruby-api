...
module Flexport
  class CreateInboundShipmentRequestV4
    attr_accessor :from
    ...
    def self.openapi_types
      {
        ...
        :'packages' => :'Array<PackageInputUnstable>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::CreateInboundShipmentRequestV4` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::CreateInboundShipmentRequestV4`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [from, items, packages, prep, shipping_plan_external_id, shipping_plan_name].hash
    end
