...
module Flexport
  class CreateParcelRequestUnstable
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::CreateParcelRequestUnstable` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::CreateParcelRequestUnstable`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [customer_parcel_id, service_level, injection_location, ship_to_address, return_address, skip_address_validation, pickup_date_time, dimensions, customer_custom_field, label_format, is_return, marketplace, custom_label_footer_fields, customs_information].hash
    end
