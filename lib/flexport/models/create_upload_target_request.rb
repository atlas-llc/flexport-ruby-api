...
module Flexport
  # The request payload for creating and upload target.
  class CreateUploadTargetRequest
    ...
    # The type of document or resource being uploaded.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::CreateUploadTargetRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::CreateUploadTargetRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @mime_type.to_s.length > 255
        invalid_properties.push('invalid value for "mime_type", the character length must be smaller than or equal to 255.')
      end
    ...
    def valid?
      ...
      return false if @mime_type.to_s.length > 255
    ...
    def mime_type=(mime_type)
      ...
      if mime_type.to_s.length > 255
        fail ArgumentError, 'invalid value for "mime_type", the character length must be smaller than or equal to 255.'
      end
    ...
    def hash
      [filename, file_size, mime_type, resource_type].hash
    end
