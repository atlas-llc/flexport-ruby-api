...
module Flexport
  # The response payload from creating an upload target, containing the form fields to be sent to S3 via the url in order to complete the file upload process.
  class CreateUploadTargetResponse
    # The URL to send the subsequent upload POST request.
    attr_accessor :url
    ...
    def self.openapi_types
      {
        :'url' => :'String',
        :'fields' => :'Array<CreateUploadTargetResponseEntry>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::CreateUploadTargetResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::CreateUploadTargetResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
