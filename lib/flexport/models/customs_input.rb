  ...
  class CustomsInput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::CustomsInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::CustomsInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'tariff_code')
        self.tariff_code = attributes[:'tariff_code']
      else
        self.tariff_code = nil
      end

      if attributes.key?(:'customs_value')
        self.customs_value = attributes[:'customs_value']
      else
        self.customs_value = nil
      end
    ...
    def hash
      [origin_country, tariff_code, customs_value, description].hash
    end
