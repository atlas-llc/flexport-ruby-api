...
module Flexport
  class DrayageOrderItemResponse
    attr_accessor :cost

    attr_accessor :customer_bol_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::DrayageOrderItemResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::DrayageOrderItemResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'customer_bol_id')
        self.customer_bol_id = attributes[:'customer_bol_id']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          cost == o.cost &&
          customer_bol_id == o.customer_bol_id &&
    ...
    def hash
      [cost, customer_bol_id, package_type, packages, master_bol_id, container_id].hash
    end
