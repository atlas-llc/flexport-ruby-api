...
module Flexport
  class FtlOrderRequest
    ...
    def self.openapi_types
      {
        :'total_weight' => :'Float',
        :'weight_unit' => :'FtlOrderRequestWeightUnitEnum',
        :'pallet_count' => :'Float',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::FtlOrderRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::FtlOrderRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @pallet_count.nil?
        invalid_properties.push('invalid value for "pallet_count", pallet_count cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @pallet_count.nil?
    ...
    def hash
      [total_weight, weight_unit, pallet_count, delivery_details, quote_id, commodity_description, customer_reference_number, pickup_details].hash
    end
