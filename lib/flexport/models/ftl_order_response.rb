  ...
  class FtlOrderResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::FtlOrderResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::FtlOrderResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @ftl_order_item.nil?
        invalid_properties.push('invalid value for "ftl_order_item", ftl_order_item cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @ftl_order_item.nil?
      true
    end
    ...
    def hash
      [type, delivery_details, quote_id, commodity_description, customer_reference_number, id, created_at, status, pickup_details, ftl_order_item].hash
    end
