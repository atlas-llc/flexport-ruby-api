...
module Flexport
  class FtlQuoteItem
    ...
    def self.openapi_types
      {
        :'total_weight' => :'Float',
        :'weight_unit' => :'FtlQuoteItemResponseWeightUnitEnum',
        :'pallet_count' => :'Float',
        :'transit_days' => :'Float',
        :'total_cost' => :'Float',
        :'original_cost' => :'Float'
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::FtlQuoteItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::FtlQuoteItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [total_weight, weight_unit, pallet_count, transit_days, total_cost, original_cost].hash
    end
