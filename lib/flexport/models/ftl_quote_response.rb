  ...
  class FtlQuoteResponse
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::FtlQuoteResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::FtlQuoteResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @ftl_quote_item.nil?
        invalid_properties.push('invalid value for "ftl_quote_item", ftl_quote_item cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @ftl_quote_item.nil?
      true
    end
    ...
    def hash
      [type, pickup_details, delivery_details, id, created_at, status, expires_at, ftl_quote_item].hash
    end
