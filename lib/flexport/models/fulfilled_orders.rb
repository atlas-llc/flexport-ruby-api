...
module Flexport
  class FulfilledOrders
    attr_accessor :orders_by_date
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::FulfilledOrders` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::FulfilledOrders`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'orders_by_date')
        self.orders_by_date = attributes[:'orders_by_date']
      else
        self.orders_by_date = nil
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          orders_by_date == o.orders_by_date &&
          delta == o.delta &&
    ...
    def hash
      [orders_by_date, delta, total_count].hash
    end
