  ...
  class FulfillmentMetricsV2
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::FulfillmentMetricsV2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::FulfillmentMetricsV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'otd')
        self.otd = attributes[:'otd']
      else
        self.otd = nil
      end

      if attributes.key?(:'ots')
        self.ots = attributes[:'ots']
      else
        self.ots = nil
      end
    ...
    def list_invalid_properties
      ...
      if @otd.nil?
        invalid_properties.push('invalid value for "otd", otd cannot be nil.')
      end

      if @ots.nil?
        invalid_properties.push('invalid value for "ots", ots cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @otd.nil?
      return false if @ots.nil?
      return false if @timezone.nil?
      version_validator = EnumAttributeValidator.new('Object', ["v2"])
    ...
    def version=(version)
      validator = EnumAttributeValidator.new('Object', ["v2"])
