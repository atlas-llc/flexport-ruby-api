...
module Flexport
  class FulfillmentMetricsV3
    attr_accessor :on_time_delivery

    attr_accessor :on_time_shipping
    ...
    def self.attribute_map
      {
        ...
        :'on_time_shipping' => :'onTimeShipping',
        :'version' => :'version'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::FulfillmentMetricsV3` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::FulfillmentMetricsV3`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'on_time_delivery')
        self.on_time_delivery = attributes[:'on_time_delivery']
      else
        self.on_time_delivery = nil
      end

      if attributes.key?(:'on_time_shipping')
        self.on_time_shipping = attributes[:'on_time_shipping']
      else
        self.on_time_shipping = nil
      end
    ...
    def list_invalid_properties
      ...
      if @on_time_delivery.nil?
        invalid_properties.push('invalid value for "on_time_delivery", on_time_delivery cannot be nil.')
      end

      if @on_time_shipping.nil?
        invalid_properties.push('invalid value for "on_time_shipping", on_time_shipping cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @on_time_delivery.nil?
      return false if @on_time_shipping.nil?
      version_validator = EnumAttributeValidator.new('Object', ["v3"])
    ...
    def version=(version)
      validator = EnumAttributeValidator.new('Object', ["v3"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          on_time_delivery == o.on_time_delivery &&
          on_time_shipping == o.on_time_shipping &&
          version == o.version
    end
    ...
    def hash
      [on_time_delivery, on_time_shipping, version].hash
    end
