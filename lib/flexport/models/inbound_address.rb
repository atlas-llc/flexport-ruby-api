  ...
  class InboundAddress
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::InboundAddress` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::InboundAddress`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @province_code.nil?
        invalid_properties.push('invalid value for "province_code", province_code cannot be nil.')
      end

      if @province_code.to_s.length > 30
        invalid_properties.push('invalid value for "province_code", the character length must be smaller than or equal to 30.')
      end
    ...
    def valid?
      ...
      return false if @province_code.nil?
      return false if @province_code.to_s.length > 30
    ...
    def province_code=(province_code)
      ...
      if province_code.to_s.length > 30
        fail ArgumentError, 'invalid value for "province_code", the character length must be smaller than or equal to 30.'
      end
    ...
    def hash
      [address1, address2, city, company, country_code, name, first_name, last_name, phone, province_code, zip].hash
    end
