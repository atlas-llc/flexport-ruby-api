...
module Flexport
  class InboundShipmentStatusChangeEvent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::InboundShipmentStatusChangeEvent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::InboundShipmentStatusChangeEvent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @shipment_id !~ pattern
        invalid_properties.push("invalid value for \"shipment_id\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if @shipment_id !~ Regexp.new(/^\d+$/)
      true
    end
    ...
    def shipment_id=(shipment_id)
      ...
      pattern = Regexp.new(/^\d+$/)
      if shipment_id !~ pattern
        fail ArgumentError, "invalid value for \"shipment_id\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [logistics_account_id, logistics_event_id, logistics_event_type, logistics_event_time, status, shipment_id].hash
    end
