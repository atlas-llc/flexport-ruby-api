...
module Flexport
  class InstalledApplication
    # Unique Identifier for the installation instance
    attr_accessor :id

    # ID of the application installed
    attr_accessor :application_id

    # Merchant's logistics account ID
    attr_accessor :logistics_account_id

    # Merchant's seller ID
    attr_accessor :seller_id

    # Installation status, such as active or inactive.
    attr_accessor :status

    # Application access token.  The token is generated by the authorization server and allows the client to access the merchant's logistics account with specified scopes.
    attr_accessor :token

    # List of scopes authorized for the application separated by space
    attr_accessor :scopes

    # A free text name for the application installed
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::InstalledApplication` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::InstalledApplication`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [id, application_id, logistics_account_id, seller_id, status, token, scopes, display_name, created_at, updated_at].hash
    end
