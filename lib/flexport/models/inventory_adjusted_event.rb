  ...
  class InventoryAdjustedEvent
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::InventoryAdjustedEvent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::InventoryAdjustedEvent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'adjustment_id')
        self.adjustment_id = attributes[:'adjustment_id']
      else
        self.adjustment_id = nil
      end
    ...
    def list_invalid_properties
      ...
      if @adjustment_id.nil?
        invalid_properties.push('invalid value for "adjustment_id", adjustment_id cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @adjustment_id.nil?
      true
    end
    ...
    def hash
      [logistics_account_id, logistics_event_id, logistics_event_type, logistics_event_time, adjustment_id, reason, adjustments].hash
    end
