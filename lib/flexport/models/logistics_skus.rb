  ...
  class LogisticsSkus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::LogisticsSkus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::LogisticsSkus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'logistics_skus')
        if (value = attributes[:'logistics_skus']).is_a?(Array)
          self.logistics_skus = value
        end
      else
    ...
    def list_invalid_properties
      ...
      if @logistics_skus.nil?
        invalid_properties.push('invalid value for "logistics_skus", logistics_skus cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @logistics_skus.nil?
      true
    end
