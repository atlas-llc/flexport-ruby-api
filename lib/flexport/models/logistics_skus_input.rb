...
module Flexport
  class LogisticsSKUsInput
    # Logistics SKUs of products to fetch.
    attr_accessor :logistics_skus

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'logistics_skus' => :'logisticsSKUs'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::LogisticsSKUsInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::LogisticsSKUsInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'logistics_skus')
        self.logistics_skus = attributes[:'logistics_skus']
      else
        self.logistics_skus = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @logistics_skus.length > 250
        invalid_properties.push('invalid value for "logistics_skus", number of items must be less than or equal to 250.')
      end

      if @logistics_skus.length < 1
        invalid_properties.push('invalid value for "logistics_skus", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @logistics_skus.length > 250
      return false if @logistics_skus.length < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] logistics_skus Value to be assigned
    def logistics_skus=(logistics_skus)
      if !logistics_skus.nil? && logistics_skus.length > 250
        fail ArgumentError, 'invalid value for "logistics_skus", number of items must be less than or equal to 250.'
      end

      if !logistics_skus.nil? && logistics_skus.length < 1
        fail ArgumentError, 'invalid value for "logistics_skus", number of items must be greater than or equal to 1.'
      end

      @logistics_skus = logistics_skus
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          logistics_skus == o.logistics_skus
    end
