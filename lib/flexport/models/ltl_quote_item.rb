  ...
  class LtlQuoteItem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::LtlQuoteItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::LtlQuoteItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @unit_cost.nil?
        invalid_properties.push('invalid value for "unit_cost", unit_cost cannot be nil.')
      end

      if @transit_days.nil?
        invalid_properties.push('invalid value for "transit_days", transit_days cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @unit_cost.nil?
      return false if @transit_days.nil?
    ...
    def hash
      [pallet_count, unit_cost, transit_days, total_cost, original_cost].hash
    end
