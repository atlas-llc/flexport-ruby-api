...
module Flexport
  class MaybeTrackingResultTimestamps
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::MaybeTrackingResultTimestamps` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::MaybeTrackingResultTimestamps`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [arrival_scan_time, attempted_delivery_time, delivery_time, estimated_delivery_time, facility_time, label_time, last_tracking_event_time, last_tracking_ingestion_time].hash
    end
