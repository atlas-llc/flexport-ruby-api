...
module Flexport
  class OrderLineItemInput
    attr_accessor :customs_unit_value
    ...
    def self.openapi_types
      {
        :'customs_unit_value' => :'MaybeMoneyInput',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::OrderLineItemInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::OrderLineItemInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'customs_unit_value')
        self.customs_unit_value = attributes[:'customs_unit_value']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          customs_unit_value == o.customs_unit_value &&
    ...
    def hash
      [customs_unit_value, logistics_sku, quantity].hash
    end
