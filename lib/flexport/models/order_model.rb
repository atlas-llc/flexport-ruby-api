  ...
  class OrderModel
    ...
    def self.openapi_types
      {
        ...
        :'state' => :'OrderState',
        :'shipments' => :'Array<PickFulfillmentShipmentExcludeKeyofFulfillmentShipmentWarehouse>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::OrderModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::OrderModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [address, cost, id, created_at, updated_at, error, delivered_at, external_order_id, line_items, promised_delivery_by_time, promised_ship_by_time, shipped_at, source, state, shipments].hash
    end
