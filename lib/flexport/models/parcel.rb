  ...
  class Parcel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::Parcel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::Parcel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @package_tracking_url.nil?
        invalid_properties.push('invalid value for "package_tracking_url", package_tracking_url cannot be nil.')
      end

      if @parcel_id.nil?
        invalid_properties.push('invalid value for "parcel_id", parcel_id cannot be nil.')
      end
      ...
      if @shipping_label_url.nil?
        invalid_properties.push('invalid value for "shipping_label_url", shipping_label_url cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @package_tracking_url.nil?
      return false if @parcel_id.nil?
      return false if @price.nil?
      return false if @service_level.nil?
      return false if @shipping_label_url.nil?
    ...
    def hash
      [carrier_code, customer_custom_field, customer_reference_id, package_tracking_url, parcel_id, price, service_level, shipping_label_url, status, tracking_code].hash
    end
