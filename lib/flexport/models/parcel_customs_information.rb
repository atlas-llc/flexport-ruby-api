...
module Flexport
  class ParcelCustomsInformation
    ...
    def self.openapi_types
      {
        :'customs_items' => :'Array<ParcelCustomsInformationItems>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::ParcelCustomsInformation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::ParcelCustomsInformation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'customs_items')
        if (value = attributes[:'customs_items']).is_a?(Array)
          self.customs_items = value
        end
      else
        self.customs_items = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @customs_items.nil?
        invalid_properties.push('invalid value for "customs_items", customs_items cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @customs_items.nil?
      true
    end
