  ...
  class ParcelTracking
    ...
    def self.openapi_types
      {
        ...
        :'errors' => :'String',
        :'event_timestamps' => :'MaybeTrackingResultTimestamps',
        :'expired_tracking_data' => :'Array<MaybeExpiredTrackingData>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::ParcelTracking` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::ParcelTracking`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [carrier_code, customer_custom_field, customer_reference_id, error_message, errors, event_timestamps, expired_tracking_data, package_tracking_url, parcel_id, shipping_label_url, status, tracking_code].hash
    end
