...
module Flexport
  ...
  class PickApiInventoryResponseExcludeKeyofApiInventoryResponseBreakdown
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::PickApiInventoryResponseExcludeKeyofApiInventoryResponseBreakdown` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::PickApiInventoryResponseExcludeKeyofApiInventoryResponseBreakdown`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@logistics_sku.nil? && @logistics_sku.to_s.length > 11
      ...
      if !@logistics_sku.nil? && @logistics_sku !~ pattern
    ...
    def valid?
      ...
      return false if !@logistics_sku.nil? && @logistics_sku.to_s.length > 11
      return false if !@logistics_sku.nil? && @logistics_sku.to_s.length < 11
      return false if !@logistics_sku.nil? && @logistics_sku !~ Regexp.new(/^D[0-9a-zA-Z]+$/)
