...
module Flexport
  # Product sync status job details
  class ProductSyncStatus
    # Unique UUID for this channel
    attr_accessor :channel_id

    # Product import job status
    attr_accessor :product_sync_status

    # Date when products were last imported
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::ProductSyncStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::ProductSyncStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'product_sync_status')
        self.product_sync_status = attributes[:'product_sync_status']
      else
        self.product_sync_status = nil
      end
    ...
    def list_invalid_properties
      ...
      if @product_sync_status.nil?
        invalid_properties.push('invalid value for "product_sync_status", product_sync_status cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @product_sync_status.nil?
      product_sync_status_validator = EnumAttributeValidator.new('String', ["COMPLETE", "PENDING", "STOPPED"])
      return false unless product_sync_status_validator.valid?(@product_sync_status)
      return false if @updated_at.nil?
      true
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] product_sync_status Object to be assigned
    def product_sync_status=(product_sync_status)
      validator = EnumAttributeValidator.new('String', ["COMPLETE", "PENDING", "STOPPED"])
      unless validator.valid?(product_sync_status)
        fail ArgumentError, "invalid value for \"product_sync_status\", must be one of #{validator.allowable_values}."
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          product_sync_status == o.product_sync_status &&
    ...
    def hash
      [channel_id, product_sync_status, updated_at].hash
    end
