...
module Flexport
  class ProvinceCoverage
    attr_accessor :province_coverage
    ...
    def self.openapi_types
      {
        :'province_coverage' => :'Hash<String, Object>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::ProvinceCoverage` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::ProvinceCoverage`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'province_coverage')
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          province_coverage == o.province_coverage
    end
