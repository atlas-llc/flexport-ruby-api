  ...
  class PurchaseQuotesRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::PurchaseQuotesRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::PurchaseQuotesRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'quote_ids')
        if (value = attributes[:'quote_ids']).is_a?(Array)
          self.quote_ids = value
        end
      else
        self.quote_ids = nil
      end
    ...
    def list_invalid_properties
      ...
      if @quote_ids.nil?
        invalid_properties.push('invalid value for "quote_ids", quote_ids cannot be nil.')
      end
