...
module Flexport
  class QuoteRequest
    ...
    def self.openapi_types
      {
        :'total_weight' => :'Float',
        :'weight_unit' => :'FtlQuoteRequestV2WeightUnitEnum',
        :'pallet_count' => :'Float',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::QuoteRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::QuoteRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [total_weight, weight_unit, pallet_count, pickup_details, delivery_details].hash
    end
