...
module Flexport
  class Report
    # <ol>   <li>PENDING: The report is still being generated</li>   <li>SUCCESS: Report has been generated, use the url field to download the report as a CSV file</li>   <li>ERROR: There was an issue while creating this report, see errorMessage field for details</li> </ol>
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::Report` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::Report`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
