...
module Flexport
  class UpdateProductInput
    ...
    attr_accessor :taxonomy_node_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Flexport::UpdateProductInput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Flexport::UpdateProductInput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'taxonomy_node_id')
        self.taxonomy_node_id = attributes[:'taxonomy_node_id']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          taxonomy_node_id == o.taxonomy_node_id
    end
    ...
    def hash
      [name, merchant_sku, price, image_url, customs_information, taxonomy_node_id].hash
    end
